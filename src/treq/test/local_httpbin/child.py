...
import httpbin  # type:ignore[import-untyped]
...
def _certificates_for_authority_and_server(service_identity, key_size=2048):
    ...
    ca_certificate = (
        ...
        .sign(
            private_key=private_key_for_ca,
    ...
    ca_self_cert = Certificate.loadPEM(ca_certificate.public_bytes(Encoding.PEM))
...
def _serve_tls(reactor, host, port, site):
    ...
    endpoint = SSL4ServerEndpoint(
        reactor, port, sslContextFactory=context_factory, interface=host
    )

    port = yield endpoint.listen(site)

    description = _HTTPBinDescription(
        host=host, port=port.getHost().port, cacert=ca_cert.dumpPEM().decode("ascii")
    )
...
def _forever_httpbin(
    ...
    """
    log.info("Starting HTTPBin Serving")
    ...
    parser.add_argument(
        "--port", help="The on which the server will listen.", type=int, default=0
    )
    ...
    description_deferred = arguments.serve(
        reactor, arguments.host, arguments.port, site
    )
...
if __name__ == "__main__":
    ...
    globalLogBeginner.beginLoggingTo([textFileLogObserver(stderr)], False)
