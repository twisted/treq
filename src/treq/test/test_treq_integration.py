...
def with_baseurl(
    ...
    def _request(
        ...
        return method(
            self.baseurl + url, *args, agent=self.agent, pool=self.pool, **kwargs
        )
...
class TreqIntegrationTests(TestCase):
    ...
    @inlineCallbacks
    def setUp(self):
        description = yield self._httpbin_process.server_description(reactor)
        self.baseurl = URL(
            scheme="http", host=description.host, port=description.port
        ).asText()
    ...
    def test_post_headers(self):
        response = yield self.post(
            "/post", b'{msg: "Hello!"}', headers={"Content-Type": ["application/json"]}
        )
    ...
    def test_failed_basic_auth(self):
        response = yield self.get(
            "/basic-auth/treq/treq", auth=("not-treq", "not-treq")
        )
    ...
    def test_timeout(self):
        ...
        yield self.assertFailure(
            self.get("/delay/2", timeout=1), CancelledError, ResponseFailed
        )
    ...
    async def test_set_cookie(self) -> None:
        response = await self.get(
            "/cookies/set", allow_redirects=False, params={"hello": "there"}
        )
        # self.assertEqual(response.code, 200)
...
class HTTPSTreqIntegrationTests(TreqIntegrationTests):
    ...
    def setUp(self):
        description = yield self._httpbin_process.server_description(reactor)
        self.baseurl = URL(
            scheme="https", host=description.host, port=description.port
        ).asText()
