        ...
        output, producer = self.getOutput(
            MultiPartProducer({
                ...
                "afield": (
                    ...
                    "text/hello-world",
                    FileBodyProducer(
                        inputFile=BytesIO("my lovely bytes"),
                        cooperator=self.cooperator))
            }, cooperator=self.cooperator, boundary="heyDavid"),
            with_producer=True)
        ...
        output, producer = self.getOutput(
            MultiPartProducer({
                ...
                "efield": (
                    ...
                    "text/html",
                    FileBodyProducer(
                        inputFile=BytesIO("my lovely bytes2"),
                        cooperator=self.cooperator)),
                "xfield": (
                    "xf",
                    "text/json",
                    FileBodyProducer(
                        inputFile=BytesIO("my lovely bytes219"),
                        cooperator=self.cooperator)),
                "afield": (
                    "af",
                    "text/xml",
                    FileBodyProducer(
                        inputFile=BytesIO("my lovely bytes22"),
                        cooperator=self.cooperator))
            }, cooperator=self.cooperator, boundary="heyDavid"),
            with_producer=True)
...
""".encode("utf-8")), output)
    ...
    def test_worksWithCgi(self):
        ...
        form = cgi.parse_multipart(BytesIO(output), {"boundary": "heyDavid"})
